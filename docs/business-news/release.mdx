...
Release Notes for the Business News data source.
...
- Plugin rebranding, the new name is the Business News Data Source.
...
- Automatic parsing of `author.name` and `media:thumbnail.url` fields break templates who uses them.

To fix it, just replace it with `author` and `media:thumbnail`. Thanks to @vfauth for testing.
...
- Works great with Business Text panel by Marcus Olsson (marcusolsson-dynamictext-panel).
- Initial release based on the Volkov Labs Data source template.
