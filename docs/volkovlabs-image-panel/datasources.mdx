...
Business Image/PDF Panel retrieves media data from the connected data source. You can use any suitable option to retrieve data in the Base64 format.

A static data source is the quickest way to get started. PostgreSQL is also an excellent choice.

## Static data source

You can use a [static data source](/plugins/volkovlabs-static-datasource) to retrieve any supported Base64 formats and render them on your Grafana dashboard.
...
<Image
  title="Business Image/PDF Panel displays media files on Grafana dashboard."
