...
The Business Table panel can export table data directly to Google Sheets using the Google Sheets API through the Infinity data source. Below is an example of configuring this integration via Google Cloud Console.
...
<Image
  title="Select project in the Google Cloud."
...
- Create and open Infinity data source.
...
We're done with setting up access to https://docs.google.com/spreadsheets. Good job. Now create a Business Table panel with the test data.
...
- Select in `Advanced options` - `External export` - `Enable` and set to true
...
:::info Status
Infinity DS sends status ok even if there is an error. It may be worth checking the Network tab if something went wrong.
:::

<Image
  title="Google Sheets data added after successful export."
...
<Image
  title="Retrieve daa in JSON format"
