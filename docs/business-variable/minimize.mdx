...
Two panels show variables that are dependent on each other. For the `State dropdown` panel, options are available depending on the selected `Country` variable. If we enable the **Allow custom values** option for the `State dropdown` panel, it causes options to appear that we do not expect to see for certain variables.
...
To avoid this behavior, we need for the `Country dropdown` panel use `Select dependent variable to reset` option.
