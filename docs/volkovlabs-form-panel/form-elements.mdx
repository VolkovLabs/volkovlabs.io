...
- Min. Sets the minimum allowed date and time.
- Max. Sets the maximum allowed data and time.
...
The Select with custom options is a dropdown where users can select one option.
...
Another example where the element is displayed when
...
There are three buttons available in the Data Manipulation form.
