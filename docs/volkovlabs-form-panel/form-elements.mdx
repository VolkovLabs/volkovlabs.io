...
Since version 4.2.0. the `Query Field` and `Field Name` fields have been relocated from the `Elements` section to the `Initial Request` options section.
...
The `Field Name` option for each Form Element is located under the code editor when the Datasource option is enabled. Specify a field name for appropriate form element from the Datasource response.
