...
Since version 4.0.0 `Sections` can be collapsible. Collapsible option available for vertical type section. Can be `Expanded` by default.
Available parameters to control expand/collapse state for custom code like `context.panel.expandSection(id)` or `context.panel.collapseSection(id)`. Full list of parameters find in ([Custom Code](/plugins/volkovlabs-form-panel/code/#parameters))

<Image
  title="Collapsible sections and options"
...
Since version 4.0.0 you can choose in the options `Time Zone` the date when the value is updated. When updating and setting the Date and Time, the date can remain in Local or be transformed to UTC.

<Image
  title="Time Zones option"
...
<BlogPost
  href="/blog/form-panel-file-upload-20240310/"
  src="/img/blog/2024-03-10-file-upload/banner.png"
  title="File upload using the Grafana dashboard."
/>
...
Below is supported starting from the version 3.2.1 and Grafana 10.1.0.
...
Colors for elements supported starting from the version 4.0.0.

:::

Colors for background, label, and label background can be set with options. Color options allow add some customization to your form elements
