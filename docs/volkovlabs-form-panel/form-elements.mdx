...
Below is supported since version 3.5.0.

:::

Get Options Code allows to set options using the code editor. The code must returns array with `{label,value}` objects. The code must be synchronous.

<Image
  title="Use values from the Get Options Code"
  src="/img/blog/2023-12-03-form-panel-3.3.0/options-from-code.png"
/>

#### Set Options received asynchronously

Get Options Code does not support asynchronous code. Options received asynchronously can be set via Initial Request.

<Image
  title="Initial request to get options"
  src="/img/blog/2023-12-03-form-panel-3.3.0/initial-req-for-options.png"
/>

Find the necessary element and update its options. Once the data has been received, update the element. Use `context.panel.onChangeElements()` to update items.
...
After that, we need to set the options from `context.panel.elements` for our element. Add the code to Get Options Code.

<Image
  title="Set options in code editor"
