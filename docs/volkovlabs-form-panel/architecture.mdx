---
tags:
  - Data Manipulation
  - Custom Forms
image: /img/plugins/volkovlabs-form-panel/form-api.png
---

import BlogPost from "@theme/BlogPost";
import Image from "@theme/Image";
import Video from "@theme/Video";

# REST API

The Data Manipulation panel can request (GET) and update (DELETE, PATCH, POST, PUT) data and configuration using REST API.

<Image
  title="Data Manipulation panel uses GET, POST/PUT request to interact with API Server."
  src="/img/plugins/volkovlabs-form-panel/form-api.png"
/>

## JSON Payload

Elements defined in the panel options are used to parse the Initial and sent in the Update requests.

<Image
  title="Elements defined in the Data Manipulation panel."
  src="/img/plugins/volkovlabs-form-panel/elements.png"
  width="80%"
/>

The JSON response of the initial request consists of the element's ids and values. The response will be parsed and update the initial values of the form.

```json
{ "name": "Name", "amount": 30, "updated": false, "step": 4 }
```

After values are updated, all form elements will create a similar payload for the Update request.

## Tutorial

<Video
  src="https://www.youtube.com/embed/SHN2S-dRIEM"
  title="How to Manipulate Data using Grafana dashboard."
/>

## NGINX

We recommend running Grafana behind NGINX reverse proxy for an additional security layer. The reverse proxy also allows us to expose additional API endpoints and static files in the same domain, which makes it CORS-ready.

<Image
  title="Grafana and Server API behind NGINX reverse proxy."
  src="/img/plugins/volkovlabs-form-panel/form-nginx-api.png"
/>

## CORS

The Data Manipulation panel may be blocked by CORS policy to access API Server. You can look for a rejection error in your browser Console window.

<Image
  title="Rejection error in the browser's Console."
  src="/img/plugins/volkovlabs-form-panel/cors.png"
/>

Unfortunately, Grafana will not be explicit. Nothing will notify you about a CORS restrictions error.

### Policies

**Same-Origin** is a policy allowing interaction with resources ONLY from the same domain, host and port. For instance, `abc.com` can retrieve data from `abc.com/page1`, but is not permitted to access `anyotherdomain.com`.

**Cross-Origin Resource Sharing (CORS)** policy is more flexible. It was created to allow different domains to interact. If CORS restrictions are off, any domain is allowed to request data. If CORS restrictions are ON, only whitelisted domains are permitted.

## Connect to API Server

We understand the risk of data manipulation and take security concerns seriously. This blog post explores three secure ways to connect the Data Manipulation panel to the API Server.

<BlogPost
  href="/blog/how-to-connect-the-data-manipulation-plugin-for-grafana-to-api-server-1abe5f60c904"
  title="How to connect the Data Manipulation plugin for Grafana to API Server."
  src="/img/blog/2022-06-12-data-manipulation-api/banner.png"
/>
