---
tags:
  - Business Forms
  - API Server
image: /img/plugins/business-forms/panel.png
---

import Code from "@theme/Code";
import Image from "@theme/Image";

# API Server for PostgreSQL

API Server on Node.js enables retrieving and updating values in the PostgreSQL database with Data Manipulation Panel.

## Features

- CORS enabled.
- Includes [SQL statements](https://github.com/VolkovLabs/volkovlabs-form-panel/blob/main/server-pg/feedback.sql) and the [Docker image](https://github.com/VolkovLabs/volkovlabs-form-panel/blob/main/server-pg/Dockerfile) to get started.

## Example

<Code
  url="https://github.com/VolkovLabs/volkovlabs-form-panel/blob/main/server-pg/server.ts"
  language="js"
/>
