...
After that, the form expects some action from the user. For example, the user can use the Reset button to implement some custom logic. In the **Reset request** section, click **–** (minus) and specify the custom logic for the Reset button in the **Custom Code** text area.
