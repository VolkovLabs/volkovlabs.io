...
### Use multiline in payload

Text Area element allows you to create text on multiple lines and add to the payload
...
After completing the element and submitting the form, you may see an error. The issue is caused by not properly processing and converting multi-line and add it to payload.

<Image
  title="Datasource update error with multiple lines"
  src="/img/plugins/business-forms/datasource-error-update.png"
/>

To avoid this behavior, please add following steps in Create Payload code editor:

- Find element refer to TextArea.
...
<Image
  title="Create payload code editor"
