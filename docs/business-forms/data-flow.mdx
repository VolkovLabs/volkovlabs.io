...
The ball is on the user side, and creating a payload for the request can be configured in a variety of ways. Keys may contain various values including nested objects.
...
After calling the update request, you will most likely get an error and in the body of the request you will see something like:

`\"matchers\": [object Object],[object Object],[object Object],[object Object]`

:::info Solution

Use `JSON.stringify` and `replaceAll('\"', '\\"')` for nested values in the payload object
:::

Since the code editor is complete user control, use interpolation and value escaping for nested objects
