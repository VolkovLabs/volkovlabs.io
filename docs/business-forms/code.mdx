...
| `context.panel.addSection(sections)`                         | Add new Sections. Added in v4.9.0.                                                                                                            | :heavy_check_mark:                                     | :heavy_check_mark:                                  |                                                                       |
| `context.panel.onChangeSections(sections)`                   | Change Sections. Added in v4.9.0.                                                                                                             | :heavy_check_mark:                                     | :heavy_check_mark:                                  |                                                                       |
| `context.panel.onChangeLayout(elements, sections(optional))` | Change Layout. Added in v4.9.0.                                                                                                               | :heavy_check_mark:                                     | :heavy_check_mark:                                  |                                                                       |
| `context.panel.removeSection(id)`                            | Remove Section. Added in v4.9.0.                                                                                                              | :heavy_check_mark:                                     | :heavy_check_mark:                                  |                                                                       |
...
The `context.panel.addSection()` handler calls the refresh panel.

If you use it in the initial request, don't forget to disable the Synchronize option.
Enabling the Synchronize option and using it together with `context.panel.addSection()` in the Initial Request will cause the panel to reload constantly.
...
```javascript
context.panel.addSection(sections);
```

**Example**

```javascript
context.panel.addSection([{ name: "Section 1", id: "id-s-1" }]);
```
...
The `context.panel.onChangeSections()` handler calls the refresh panel.

If you use it in the initial request, don't forget to disable the Synchronize option.
Enabling the Synchronize option and using it together with `context.panel.onChangeSections()` in the Initial Request will cause the panel to reload constantly.
...
```javascript
context.panel.onChangeSections([{ name: "Section 1", id: "id-s-1" }]);
```
...
The `context.panel.onChangeLayout()` handler calls the refresh panel.

If you use it in the initial request, don't forget to disable the Synchronize option.
Enabling the Synchronize option and using it together with `context.panel.onChangeLayout()` in the Initial Request will cause the panel to reload constantly.
...
```javascript
context.panel.onChangeLayout(elements, sections);
```
...
context.panel.onChangeLayout(
  ...
  [{ name: "Section 1", id: "id-s-1" }]
);
```

**Arguments**

- `elements` _Array_ . If element(s) contain `section` property, section would be add to layout from property if section not specified in editor.

- `sections` _Array_ . Optional argument. Change current sections if already specified. or add new sections, if sections not specified
