...
The Business Charts panel supports the imports.

:::

You can use the built-in functionality via `context`, for example `context.ecStat`, to draw graphs.

Another alternative method is to import the libraries and apply them to your eChart instance.
...
In this example we use `setOption(options)` method in `then` method

<Image
  title="Scatter type graph with imported ecStat library"
  src="/img/plugins/business-charts/imports/scatter-import-example.png"
/>
...
For next example we use JSON API datasource and data example from Apache ECharts website.

Code example since version 6.4.0:

```js
const dataSetTest = context.panel.data.series[0].fields[0].values;
...
<Image
  title="Boxplot type graph with imported ecSimpleTransform library"
  src="/img/plugins/business-charts/imports/boxplot-import-example.png"
/>

Code sample for versions lower than 6.4.0:
 ...
 * echarts-simple-transform import
 */
import("https://esm.sh/@manufac/echarts-simple-transform@2.0.11").then(
  ({ default: ecSimpleTransform }) => {
    context.echarts.registerTransform(ecSimpleTransform.aggregate);
