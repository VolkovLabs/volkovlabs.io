...
Grafana 9 supports Internationalization, which is not fully provided to plugins yet. Meanwhile, we can use `getLocale()` method to get a value for the selected locale and show phrases from a defined dictionary.
