...
Use **All rows** or **All data** template to ensure better performance. With **Every row**, the **Content** will be applied to every row of retrieved data. Even though the data frames of the specified data source are not used, still the plugin runs the code as many times as the number of rows is retrieved. 

To better understand this, connect to the -- Grafana --/Random walk data source and select **Every row** template. There is a high chance your dashboard will load with a noticeable delay. 
