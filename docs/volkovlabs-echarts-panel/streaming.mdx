---
description: Streaming allows real-time data updates using streaming Data Sources and Grafana Live.
tags:
  - Apache ECharts
  - Streaming
image: /img/plugins/volkovlabs-echarts-panel/live.png
---

import Code from "@theme/Code";
import Image from "@theme/Image";
import Video from "@theme/Video";

# Streaming

:::info Version

Streaming is supported by the Apache ECharts visualization panel starting from version 4.1.0.

Apache ECharts version 5.0.0 fixed the re-render issue for live data.

:::

Streaming enables real-time data updates using streaming data sources and Grafana Live.

## Grafana Live

Grafana Live is a real-time messaging engine introduced in Grafana v8.0. With [Grafana Live](https://grafana.com/docs/grafana/latest/setup-grafana/set-up-grafana-live/), you can push event data to a frontend as soon as an event occurs.

<Image
  title="Apache ECharts allows displaying messages from Grafana Live real-time engine."
  src="/img/plugins/volkovlabs-echarts-panel/live.png"
/>

### Example

<Code
  url="https://github.com/VolkovLabs/volkovlabs.io/blob/main/docs/volkovlabs-echarts-panel/live.js"
  language="js"
/>

## WebSocket API

To stream real-time data, we used the [WebSocket Data Source for Grafana](https://grafana.com/grafana/plugins/golioth-websocket-datasource/).

<Image
  title="Streaming real-time events from Websocket Data source."
  src="/img/plugins/volkovlabs-echarts-panel/websocket.png"
/>

### Server

Example of Node.js WebSocket Server implementation.

<Code
  url="https://github.com/VolkovLabs/volkovlabs-echarts-panel/blob/main/websocket/server.js"
  language="js"
/>

### Provisioning

WebSocket API Data Source can be provisioned using configuration files or in the Grafana UI.

<Code
  url="https://github.com/VolkovLabs/volkovlabs.io/blob/main/docs/volkovlabs-echarts-panel/websocket.yml"
  language="yaml"
/>

### Example

<Code
  url="https://github.com/VolkovLabs/volkovlabs.io/blob/main/docs/volkovlabs-echarts-panel/streaming.js"
  language="js"
/>

## Gauge

Another example is how to display Grafana Live data using Gauge.

<Image
  title="Display gauge with the live data in real-time."
  src="/img/plugins/volkovlabs-echarts-panel/gauge.png"
/>

### Example

<Code
  url="https://github.com/VolkovLabs/volkovlabs.io/blob/main/docs/volkovlabs-echarts-panel/gauge.js"
  language="js"
/>
