---
tags:
  - Apache ECharts
image: /img/plugins/volkovlabs-echarts-panel/dom.png
---

import Code from "@theme/Code";
import Image from "@theme/Image";
import Video from "@theme/Video";

# ECharts Instance

The `echartsInstance` is a created instance (container) of the Apache ECharts library. It allows you to do the following:

- Get the width and height of the ECharts container.
- Get the DOM (Document Object Model) to include additional elements.
- Resize the chart size.
- Handle events.
- Update options, etc.

For more details, please the documentation for [echartsInstance](https://echarts.apache.org/en/api.html#echartsInstance).

## Scale when resizing

To scale the content for fitting the resized panel, you need to use the instance methods to retrieve the width and height of the panel.

```javascript
  graphic: {
    type: "image",
    style: {
      image: `data:image/svg+xml;utf8,${SVG}`,
      width: echartsInstance.getWidth(),
      height: echartsInstance.getHeight(),
    },
  },
```

## Create DOM elements

You can add elements (buttons, checkboxes, select boxes, etc.) to the DOM (Document Object Model) using the instance.

### Button

You can add a button with an on-click event handler on top of Apache ECharts.

<Image
  title="Add a button to handle events in the Apache ECharts container."
  src="/img/plugins/volkovlabs-echarts-panel/dom.png"
/>

<Code
  url="https://github.com/VolkovLabs/volkovlabs.io/blob/main/docs/volkovlabs-echarts-panel/dom.js"
  language="js"
/>

### Select

Add a select box with an on-change event handler.

<Image
  title="Add a select box with options and update Apache EChart options based on the user's selection."
  src="/img/plugins/volkovlabs-echarts-panel/select.png"
/>

<Code
  url="https://github.com/VolkovLabs/volkovlabs.io/blob/main/docs/volkovlabs-echarts-panel/select.js"
  language="js"
/>
