...
Access restriction ensures that only trusted users are allotted Admin and Editor privileges.

Users with a Viewer role are not able to open panels in edit mode, hence, their ability to even accidentally create a hazard is decapitated.
...
The [Business Input data source](/plugins/business-input/) allows to set values using JavaScript Value Editor, which can be disabled in the data source options.
