...
- **Scheduled**. The alert is scheduled, but never run yet. This status is assigned right after the alert is created or modified by the user/API.
...
After any mo, the alert status changes to **Scheduled**.

Reference the flow schema to get a better understanding of how statuses change [here](/engine.mdx)
