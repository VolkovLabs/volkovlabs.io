...
The **Overview** page, in addition to the basic engine statistics such as version, status (ok/error), a number of alert rules and actions, features the Alert History and high availability state.

<Image
  title="A Business Engine Overview page"
...
The **Alert rules** page is a part of the [Business Alerting](/big/alerting/) feature available in all Business Engines.

To learn more about alert rules, please refer to this documentation's [**Alert rules**](/big/alerting/manage-alert-rules/) section.

## Actions

The **Actions** page helps to configure actions, which are the set of steps that are triggered by an expected event. The most common scenario is when an action is triggered after an alert rule is broken, but it is just one of the possible scenarios.

To learn more about actions, please refer to this documentation's [**Actions**](/big/studio/actions/) section.

## Environment

All environment variables are displayed on the **Environment** page.

The **Grafana** -> **Version** informational detail makes the work with Grafana version compatibility more transparent.
...
The **Settings** page contains all the information you provided while creating the Business Engine plus one extra parameter - **Update Engine license**.
