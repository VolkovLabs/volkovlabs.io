...
To access the alert configuration in the Business Studio, select the Business Engines from the main screen, then switch to the **Alert rules** tab. From there, either use the existing alert rule from the list to reconfigure or create a new rule by clicking on the **+ Add** button.
