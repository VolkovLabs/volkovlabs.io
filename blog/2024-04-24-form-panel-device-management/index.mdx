...
Most of the time, Grafana is considered as a superb system monitoring and observability tool. 

However, Grafana has much more to offer. 
...
This article describes a use case demonstrating how a user can control an NVidia Orin device from a Grafana dashboard. To spice things up a bit, we enhanced the device by mounting the following:
 - a custom LED Monitor to display IP address, MAC, and device names,
 - LED lights to indicate a status by various types of blinking. 

<Image
  title="The NVidia Orin device with a custom LED monitor and LED lights."
  src="/img/blog/2024-04-24-form-panel-device-management/orin.png"
/>
...
The final Grafana dashboard contains two Data Manipulation panels: 
 - `LED Monitor` with **Tier** and **Interval** as read-only metrics. A user can change the device **State** between on and off using the drop-drop element.
 - `LED Lights` with **Count** and **Intensity** as read-only metrics. A user can change between different **Modes**. Here, a mode is how the LED lights beam.
...
Down below in this article, we describe in detail how to configure the `LED Lights` Grafana panel. The configuration of the other panel, `LED Monitor` is very similar. 
...
This makes the Data Manipulation plugin the ideal candidate for managing your devices from a Grafana dashboard.

Below we will review in detail how we configured the `LED Lights` Data Manipulation panel on our dashboard.

### Data Source and the Initial Request

We start with choosing the NVidia Orin data source and selecting the appropriate **Request** type which in our case is the `Get LEDs`.
...
When **Synchronize with data** is **Enabled** with every such refresh, the data populating all Data Manipulation form elements is re-fetched from the data source. That means if the refresh process catches a user in the middle of the editing, all newly entered values are going to get lost. 

To avoid that or in other words, to prevent the Data Manipulation form elements to re-fetch the data with every dashboard refresh, we set **Synchronize with data** to **Disabled**.
...
We also need one **Select with custom options** type to allow users to modify the current LED Light blinking mode. For this type, we similarly to the previous ones specify **Query Field** and then using the **Option Source** set to **Custom** create the listing of all options we need. 
...
Please, note that we also call the Initial Request here to ensure the form elements (mode, in this case) are updated timely with the user actions. 

## Final result

After all described above steps, we ended up with the Data Manipulation form that displays the current device stats allowing users to modify one data element - the device mode.
