...
The plugin is submitted for review.
:::
...
In the Event section, a new <b>Location</b> parameter is available for Big Calendar type only. It is displayed in two places. First is following the <b>Text</b>, separated by a colon character(:). The second is in the Event Details, a popup window appearing after you click on the event in the calendar panel.
...
This feature ensures to pick a text color to always contrast well with the backgound color.
It is available for Big Calendar type only. 
...
Starting from this release, the Calendar panel will automatically synch with the TimeZone specified for the dashboard. Previously, it synched with the Browser Time Zone.

<Image
  title="The Calendar panel automatically syncs with the dashboard Time Zone."
