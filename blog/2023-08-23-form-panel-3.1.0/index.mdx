...
A custom payload code editor is available for initial requests to the Data Source. When the data source is selected, you can specify a custom payload to execute the SQL statement `rawSql` and any other required property required for the specific data source.
...
The support for Data Sources was one of the most requested community features. Starting from version 3.1.0 you don't need a Rest API server to interact with databases or storage configured as Data Sources.
...
You can input the field name returned from the data source for any element to get the latest value, which can be modified and sent in the payload to update.
...
Update request payload should be created in the custom code according to the Data Source specification. For SQL database it's `rawSql` and `format`:
...
In Data Manipulation v3 the `onChange()` is required to update the element values in the local state.
