...
An RSS data source was implemented with the recently released [Data Source Template](https://github.com/volkovlabs/volkovlabs-abc-datasource). This template is similar to the panel plugin template introduced in the previous articles. Business News Data Source retrieves a feed and returns data frames to display news in the [Business Text panel](/plugins/volkovlabs-dynamictext-panel) or other custom panels.
...
We decided to develop the [Business News data source](/plugins/volkovlabs-rss-datasource/) without the backend using [our data source plugin template](https://github.com/volkovlabs/volkovlabs-abc-datasource). Similar to the panel plugin template, this template comes preconfigured with Docker Compose, initial provisioning, and continuous integration with test coverage settings.
...
You can find more information about the Business Text Panel plugin and various helpers in our [documentation](/plugins/volkovlabs-dynamictext-panel).
