...
# Nested objects, pagination options, and E2E tests in Business Table 1.5.0
...
- Nested objects (add, edit, delete rows in a cell). **Experimental**
...
## Nested objects (add, edit, delete rows in a cell)

### What is the Nested objects feature

Simply put, this feature allows an end-user to add, edit, and delete rows associated with a table cell. Until we have a better name, we call the **nested objects** or **comments**.

Developed to meet the specific requirements of a commercial client, this feature, even in its current form, holds significant potential for the broader open-source community.

In the case of good feedback, we might expand the nested objects/comments feature to make it more flexible and configurable. 

For now, you are welcome to experiment within the existing limits.
...
Please note that the user can add/edit only two columns (title and description) in the current feature design. 
...
In the Business Table options -> **Layout** ensure to:
 ...
 - Specify the **Object**, which is a column name for WHAT????.
...
In the **Nested objects** category:
 ...
 5. Create a query. In this example, I use the SQL query, where .....
...
Configuring a comment card means identifying data frame column names for display. In this release, you can specify four column names. An end user can enter only two from the dashboard: **title** and **description**.

<Image
  title="Configure a comment card."
