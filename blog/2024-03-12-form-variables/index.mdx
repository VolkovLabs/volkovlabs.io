...
# Sync dashboard variables with Data Manipulation plugin

This blog post was initiated by a [particular question](link) from the Community.

While working on a detailed answer, we realized that the solution to this question covers multiple hot Grafana topics and might thus be of interest to many Community members.
...
We will start by describing the product in its current state.

- A Grafana dashboard with three hierarchical dashboard variables.
- Any analytical chart that rebuilds according to the selected dashboard variable combination. The nature of the chart is unimportant.

<Image
  title="The product in the current state."
...
Here is what the configuration of the dashboard variables looks like. We usually use the [Static Data Source](/plugins/volkovlabs-static-datasource) in our demo projects. We do it for many reasons, including making it straightforward to follow our steps.
...
## Requirements for the enhanced state
...
We add the Data Manipulation panel and create three form elements with the type **Select with Custom Options**: facility, room, and bench.
...
- The selectable options for each **Select with Custom Options** form element should be populated dynamically and be identical to the current dashboard variables choices.
  ...
  Thus, we use the same approach, namely the [Static Data Source](/plugins/volkovlabs-static-datasource). As shown below, we create three queries: Facility Options, Room Options, Bench Options.
...
In this example two sets of filters, in the dashboard variables and the Data Manipulation plugin, are identical. This comes as an initial requirement. Note, that technically those two lists might differ. However, we are not aware of a good use case for not alike lists.
...
To facilitate the Initial load synchronization between dashboard variables and the Data Manipulation form elements we do the following:
...
The opposite synchronization is already in place since the values for the current Data Manipulation form elements are taken from the dashboard variables.
...
JavaScript code to enable the **Submit** button.
