...
With the <b>Local</b> request mode, the Grafana HTTP API data source connects to the local configuration storage using the same Grafana privileges a user has been assigned during login and, thus, simplifies the user management.

The <b>Local</b> request mode has no parameters. 

<Image
  title="The new request mode, Local, has no parameters."
...
Starting from this release, the HTTP API data source digests the <b>Values</b> field from the stored Annotations.

<Image
  title="One more element is digested from the stored Annotations - Values."
...
<b>Request</b> is a drop-down parameter that appears when you connect your panel to the HTTP API data source. In addition to all existing options, we added <b>Organization Users</b>. The picture below illustrates the new possibility. 

<Image
  title="The new option in the Request parameter - Organization Users."
