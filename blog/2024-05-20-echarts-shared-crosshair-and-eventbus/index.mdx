---
authors: [vitali]
slug: echarts-shared-crosshair-and-eventbus-2024-05-20
...
# Panel communication via EventBus
...
The solution in this article integrates the open-source Business Charts plugin with the Crosshair and EvenBus functionalities to have Native and Community panels talking to each other. The panels' interconnectivity makes data analysis more visual and supports Grafana as a top-notch visualization tool.
...
A crosshair is usually two lines crossed at the point of focus.

:::

In general, the Crosshair feature helps to highlight the selected data points on the graph. Some of the native Grafana panels have crosshair built-in from the start. For instance, TimeSeries displays it right off the bat. When a user hovers a mouse over the graph, the two dotted perpendicular lines appear.

<Image
  title="The built-in Time Series crosshair."
...
Below is an example of two Time Series on the dashboard with the **Shared crosshair** turned on. The Data Sources for both are set to **--Grafana--** -> **Random Walk**.
...
## Crosshair on the Business Charts Panel
...
Now let's review how to display a shared crosshair on not native panels, or, in other words, how to faciliate panels' interconnectivity. I will use the [Business Charts panel](/plugins/volkovlabs-echarts-panel/) as an example of a Community plugin.

### Step 1. Add two Business Charts panels

Add two Business Charts panels on the existing dashboard. The codes for both panels are identical.

The data source is **--Grafana--** -> **Random Walk** for both panels.
...
With two Time Series and two Business Charts panels, the dashboard looks as below. The **Shared crosshair** feature is enabled, so, we can see a synchronized crosshair on the Time Series panels. The Business Charts are not yet linked to the Time Series and, thus, no crosshair for them yet.
...
### Step 2. Configuring EventBus for panels' interconnectivity

Grafana supports the EventBus approach. This is when you subscribe to the application's events and describe an action in case the event occurs.

:::info One possible solution

Using EventBus is one way to facilitate an interaction between visualization panels. The other approach could be by using dashboard variables, but this topic is not covered in this article.

:::

My team started working on documentation for [Grafana EventBus](/plugins/volkovlabs-echarts-panel/eventbus). As of today, you can find the code demonstrating how to subscribe to an event there.

:::info Synchronize in both directions

Please note that the synchronization between the two panels consists of a reaction to the changes on both panels.

- When a mouse hovers above the Time series, we must update the Business Charts.
- When a mouse hovers above the Business Charts, we must update the Time Series.

:::

#### Step 2.1. Modify the Business Charts panels following the Time Series change.
...
#### Step 2.2. Modify the Time Series panels following the Business Chars change
