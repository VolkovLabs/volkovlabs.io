...
The solution in this article integrates the open-source [Business Charts](/plugins/volkovlabs-echarts-panel) panel with the Crosshair and EventBus functionalities to have native and community panels talking to each other. The panels' interconnectivity makes data analysis more visual and supports Grafana as a top-notch visualization tool.
...
<Image
  title="The native Time Series crosshair functionality."
...
<Image
  title="Two Time Series panel with shared crosshair functionality."
...
Now let's review how to display a shared crosshair on community panels, or, in other words, how to faciliate panels' interconnectivity. I will use the [Business Charts panel](/plugins/volkovlabs-echarts-panel/) as an example of a Community plugin.
...
- When a mouse hovers above the Time series, we listen for events in the Business Charts.
- When a mouse hovers above the Business Charts, we send events for the Time Series panel.
...
const subscription = context.grafana.eventBus.subscribe(
  ...
  (data) => {
    /**
     * for Random Walk 2 it should be not equal to "echarts-walk-2"
...
/**
 * Return the ECharts configuration object sas extended object
 * contain unsubscribe option
 * The Business Charts Panel supports the extended result object since version 5.0.0.
 */
return {
...
- Support our projects by starring the repository.
