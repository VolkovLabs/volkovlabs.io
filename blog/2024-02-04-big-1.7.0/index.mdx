...
Starting from this release, you can use the Swagger REST API to work with our BI Engine. The detailed instructions are available at [REST API](/big/api/) section of our documentation.
You can learn and try simple requests there.
...
Grafana supports a wide range of data sources. The most popular to work with while configuring alerts is the PostgreSQL data source. From the beginning, we ensured the compatibility between the BI Engine and the PostgreSQL data source. We are actively working on ensuring many other data sources are compatible.

However, in the case when your particular data source is not yet supported, we allow to specify variable values directly on the Add/Edit alert drawer. 
