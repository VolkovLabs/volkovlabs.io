...
Grafana 9 added Internationalization, which is not exposed to plugins yet. Meanwhile, we can use the `getLocale()` method to get a value for the selected locale and show phrases from a defined dictionary.
