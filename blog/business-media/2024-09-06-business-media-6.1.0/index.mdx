...
The Business Media panel reached an inspiring number of downloads! 3 000 000! Are we surprised and honored? Yes! Are we proud? Oh yes!
...
The media files can be loaded from the URL link which makes video streaming as accessible as ever.

To try it out, simply copy the link of a video, specify media type and field name in the **Media Sources** parameter.
...
:::warning breaking change
Following Grafana 11 X's new features, we removed previously existing image links and allowed using a new **Data links** feature instead. Be sure to update your Business Media panel options accordingly. Otherwise, your links might NOT work correctly.
:::
...
You can specify a poster image (thumbnail) file for your video files. Use the **Video/Audio**->**Poster Image** parameter for that.

A poster image (thumbnail) is an image that is shown before the user clicks on the play button (unless the **Auto play** is enabled).

The image can be in the base64 format or a link to the external web resource. 

URL image link from the example below:
``` link
...
<Image
  title="Original with scroll size mode."
