...
- Promtail collects, processes log files from Nginx and pushes to Loki.
...
The configuration file is based on the default file shipped with `2.7.1` release. We updated the configuration to increase the number of maximum outstanding requests to accommodate the Grafana dashboard.
...
The configuration file is based on the default file shipped with `2.7.1` release. We updated the configuration for Nginx analytics log files to add labels `job`, `host`, `agent` and watch for `analytics.log` files.
