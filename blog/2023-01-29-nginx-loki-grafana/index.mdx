...
In this weekend' project, we are interested in Content metrics - Who, What, and from Where requested content from blog and documentation on our website, rather than Technical - how many log files were parsed, size, error requests, etc. To achieve this goal, we configured Nginx, Loki, Promtail to work together and show all required metrics on a Grafana dashboard.
...
You can spend extra time configuring custom dashboards and reports, but I still feel we are missing user activities. One of the reason, reporting requests are blocked on firewalls, security devices, and VPN devices. Collecting user activities directly from web server will help to avoid connectivity issues.

## Log flow

Let's examine each component functions and take a closer look at each component one-by-one.

- Nginx serves website and produce log files.
- Loki aggregates log files.
- Promtail collected and processes log file from Nginx and push to Loki.
- Grafana displays content and technical metrics from Loki using Loki data source.

## Nginx

Nginx is a web server that can also be used as a reverse proxy, load balancer, and cache. We use it in front of all our projects, running in Docker, and it's installed on the host for optimal performance.
...
We also added additional configuration to the main configuration file `nginx.conf` to get as clean input data as possible and avoid additional processing in Promtail and Grafana:
...
- Replace empty values in `geoip_country_code` with `Unknown`, which we display with the pirate flag on our dashboard similar to other countries.
...
When using the Nginx web server, server blocks can be used to encapsulate configuration details and host more than one domain on a single server. In the configuration file for the website we added additional logging file `analytics.log` using described JSON format.
...
Loki is a horizontally scalable, highly available, multi-tenant log aggregation system inspired by Prometheus. It does not index the contents of the logs, but rather a set of labels for each log stream. It is designed to be very cost effective and easy to operate.
...
Loki, Promtail, and Grafana containers are distributed on separete hosts, and we run them in the network host mode.

### Configuration

The configuration file is based on the default file shipped with `2.7.1` release with additional configuration to increase the number of maximum outstanding requests to accommodate the Grafana dashboard.
...
Configuration file is based on the default file shipped with `2.7.1` release with additional configuration for Nginx analytics log files to add labels `job`, `host`, `agent` and watch only described `analytics.log` files.
...
A Loki data source is preinstalled in Grafana. We used default configuration with URL `http://localhost:3100` to connect to the private Loki instance.
...
The first part of the dashboard provides Content metrics - Who, What, and from Where requested content.
...
- Geomap displays number of request for each Geo location, except empty values replaced with `Unknown`.
- Requests panel displays total number of requests.
- Visitors per Hours displays number of unique remote IP addresses per hour.
- A list of contries presented as sorted table with values mapping. List of countries can be updated if required.
- Referers sorted table displays the location from where requested resources are being used.
