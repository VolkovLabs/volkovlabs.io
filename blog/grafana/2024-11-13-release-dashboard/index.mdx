...
tags: [Solution, Business Suite, Grafana Crash Course]
...
# Create a Business Dashboard in Grafana for release management (including data entry)
...
- [Manual Entry form (it is shown in the Data Flow section below)](#manual-entry-form)
...
- [Manual entry](#manual-entry-form). When the release is published, release manager manually enters all required information via the Grafana dashboard using the [Business Forms](/plugins/business-forms/) panel.

<Image
  title="The Grafana dashboard to manually enter release details into the data source."
...
<Image
  title="Manual data entry form (Business Forms) panel."
...
<Image
  title="Manual data entry form (Business Forms) panel configuration."
...
The **Update Request Payload** parameter becomes availabe when the **Update Request**->**Update Action**->**Data Source** is selected.
