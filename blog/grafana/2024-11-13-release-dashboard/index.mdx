...
tags: [Grafana Crash Course, Business Suite]
...
This artcile aims to be one example. [Release dashboard](https://release.volkovlabs.io) is an internal Volkov Labs project available for the public to view. We use it daily to monitor our plugin release schedule and community interest by using various metrics such as number of downloads, open issues, etc.
...
- Manual entry. When the release is published, release manager manually enters all required information via the Grafana dashboard using the [Business Forms](/plugins/business-forms/) panel.
...
As you can see on the [data flow](#data-flow) schema above, we primarely use three tables: [grafana](#grafana-table), [github](#github-table), and [releases](#releases-table).
...
The panel shows our products: [Business Alerting](/big/alerting/), open-source [Business Suite](/plugins/), and the [Environment data source](/grafana/environment/).
...
| 3. Apply [transformation](/grafana/transformations/) | We use the **Group by** transformation to calculate the number of supported plugins by each Grafana version |
| 4. Extracted data review                             |                                                                                                             |
| 5. Select a visualization panel from a list          | [Business Charts panel](/plugins/business-charts/)                                                          |
| 6. Configure panel properties                        | We use the **Code** Editor mode. See the code below                                                         |
