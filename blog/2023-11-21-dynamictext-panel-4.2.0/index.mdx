...
- Added **JavaScript->After Content Ready** option to execute after the code in the content is run.
...
This is a new parameter to specify another JavaScript code. This one will be executed after the **Content** option runs. 

Another important change to point out is we reorganized the UI positioning of code options for a more intuitive display. 
...
<Image
  title="New order of the coding parameters."
...
<Image
  title="Show/hide the parameters you need using the Dynamic Text section."
  src="/img/blog/2023-11-21-dynamictext-panel-4.2.0/show-hide.png"
  lazy={false}
/>

Below are two schemas of the rendering order in the Dynamic text plugin. They represent the same thing. The first is oriented toward a general public and the second is targeted to answer developers' questions. 

### The Rendering Order General

<Image
  title="The Rendering Order General."
...
In the **JavaScript->before rendering content** option a user can describe(register) handlebars which are functions to use in the rest of the code. 
