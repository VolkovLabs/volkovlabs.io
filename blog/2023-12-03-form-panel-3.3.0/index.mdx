...
- **Confirmation Window** element names are now customizable.
- Usability improvement: any Code Editor automatic size change.
...
This new element type allows you to add links on your Data Manipulation form. The link text is customizable using the **Link Text** option.
...
Previously all types with Custom options supported only options entered manually during the configuration. Starting from this release, you can reference a query from any configured data source.

You can specify the **Label** and **Value**.
...
Now you can specify a data source for the **Reset Request** and reference its fields in the **Custom code** if needed.
...
Starting from this release, you can customize all name elements using the **Confirmation Window** section.
...
Any option containing code (for instance, **Show if returned value is true**, **Custom code**) changes its size(number of rows) following the number of code rows in it. The smallest size is 10 rows of code which means the size will not shrink to be less than 10 rows. The size can increase up to 2000 rows of code.
