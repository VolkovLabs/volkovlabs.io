...
tags: [Dynamic Text, D3, Community]
...
This file can then be imported into the Grafana's Dynamic Text plugin developed by [Volkov labs](https://volkovlabs.io), which allows you to display and interact with the chart directly within the dashboard.
...
To take things a step further, you can use the [Dynamic Text plugin's ability](https://volkovlabs.io/plugins/volkovlabs-dynamictext-panel/external/) to link external JavaScript libraries and CSS stylesheets to add additional functionality to the chart. 

For example, you could use the popular data visualization library [D3.js](https://cdnjs.com/libraries/d3) to display real-time metric values directly on the flow chart. The Dynamic Text plugin also includes hooks for controlling data manipulation before and after the content is rendered, giving you even greater flexibility over how the metrics on the chart are displayed and changed based on data in real time.
...
To learn more about these features and how to use them, check out the official documentation for the Dynamic Text plugin [here](https://volkovlabs.io/plugins/volkovlabs-dynamictext-panel).
...
<Image
  title="Steps to incorporate d3 into a Grafana dashboard using the Dynamic Text pluign."
...
The latest version of the Volkov Labs Dynamic Text panel opens up a lot of possibilities that we can achieve with the Grafana dashboarding tool.
