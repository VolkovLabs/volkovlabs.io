...
We actively support all our plugins and timely update them to be compatible with the latest Grafana versions. You always can follow the [release schedule and other plugin statistics](https://release.volkovlabs.io) on one of our public projects. 

When we discuss Grafana, our focus is on what this platform is capable of to deliver to an end-user.
That's why we provide detailed plugin documentation and specific use cases with step-by-step solutions. Whether it's for monitoring system performance or analyzing business data, our aim is to show that building with Grafana can be both time and cost-efficient. More importantly, its functionality can be fine-tuned to your particular needs, ensuring you have all the answers without unnecessary features. 

Currently, the Business Suite consists of 9 uniquely positioned plugins, each developed for its niche of tasks.
 
- [Business Media panel](/blog/business-suite-20240802/#business-media-panel), 
- [Business News data source](/blog/business-suite-20240802/#business-news-data-source), 
- [Business Text panel](/blog/business-suite-20240802/#business-text-panel), 
- [Business Satellite data source](/blog/business-suite-20240802/#business-satellite-data-source).
- [Business Forms panel](/blog/business-suite-20240802/#business-forms-panel), 
- [Business Charts panel](/blog/business-suite-20240802/#business-charts-panel), 
- [Business Calendar panel](/blog/business-suite-20240802/#business-calendar-panel),
- [Business Input data source](/blog/business-suite-20240802/#business-input-data-source),
- [Business Variable panel](/blog/business-suite-20240802/#business-variable-panel).

We are expecting the first release of the [Business Table](/blog/business-suite-20240802/#business-table-panel) towards the end of August.

## Business Media panel

About three years ago, we became part of the vibrant Grafana community. Our first project was an AI image recognition, counting the cars on a busy highway. Grafana was a significant asset, but we faced a unique challenge. To address this, we developed our first community plugin. This panel allowed us to visualize base64 encoded images, allowing us to monitor the real-time movement of cars on our Grafana dashboard. 

Many other formats have been added since the first image format, including PDF, video and audio files. Now the plugin is known as the [Business Media](/plugins/volkovlabs-image-panel/) panel with almost 3,000,000 downloads worldwide. 
...
Eventually, we submitted this new data source for the Grafana team's review and were excited to have another plugin added to the Grafana catalog. Now, it is known as the [Business News](/plugins/volkovlabs-rss-datasource/) and has nearly 800,000 downloads.
...
:::note
If you’re unable to display an RSS feed using the news visualization, you can try using the community RSS/Atom data source plugin in combination with the Dynamic text community panel. This will allow you to display the RSS feed in a different way.
:::
...
The [Business Text](/plugins/volkovlabs-dynamictext-panel/) panel has become the most popular Volkov Labs plugin with more than 7,000,000 downloads. The plugin truly cuts through many corners with its ability to integrate external JavaScript libraries. 

With that functionality, beloved panel like Flowchart can have a [second life](https://volkovlabs.io/plugins/volkovlabs-dynamictext-panel/external/#flowchart).

Currently, the plugin is called the Business Text, but it could as well be named a swiss army knife for Grafana.

Read the [feedback](https://community.grafana.com/t/recommendation-business-text-panel-for-javascript-shenanigans/125839) about the Business Text plugin written by one of the community members.

## Business Satellite data source

Shortly after the successful Business News and Business Text plugin combo, we started working on another plugin. Initially, we called it the HTTP API data source, but presently, it is known as the [Business Satellite](/plugins/volkovlabs-grapi-datasource/) data source. It facilitates access to the Grafana configuration database, the central repository for all Grafana settings, alerts, and annotations. 
...
We are open to new suggestions. Please contact us if your task requires working with something else. 
...
At first glance, the [Business Forms](/plugins/volkovlabs-form-panel/) options section might seem complex. To address this, we've created a data flow schema. This visual representation clarifies how the options interact, answering most of the questions we've received and providing a clear path forward. 
...
In the documentation to this plugin, we share a ready-to-use code for [5 basic API servers](/plugins/volkovlabs-form-panel/servers/):

- [InfluxDB](/plugins/volkovlabs-form-panel/servers/influx/),
- [JSON](/plugins/volkovlabs-form-panel/servers/json/),
- [MySQL](/plugins/volkovlabs-form-panel/servers/mysql/),
- [Node-RED](/plugins/volkovlabs-form-panel/servers/nodered/),
- [PostgreSQL](/plugins/volkovlabs-form-panel/servers/postgresql/).
...
Every plugin has a specific role in our Business Suite, in addition to that, the [Business Charts](/plugins/volkovlabs-echarts-panel/) has a special place in our hearts. With this plugin you can build highly customizable charts of any kind. From simple lines, bars, radar, theme rivers, to maps and 3D simulations. The graphs can be dynamic or transforming from one to another. 
...
To get you started faster, we host the [demo project](https://echarts.volkovlabs.io/) where you can review the code for each chart type.
...
<Youtube id="CvLqyY2fQfo" title="Business Calendar 3.1.0 | Overview and detailed tutorial | Display dates and time in Grafana" />

Among many [Business Calendar](/plugins/volkovlabs-calendar-panel/) panel features are:
...
This plugin has started as a helping hand in visualization panel development. In the first release, it allowed users to manually create a data frame with the needed data types for quick and simple visualization panel testing. 

Currently, you can find this plugin enriched with the JavaScript module added to simplify the data emulation. The dashboard variables are supported. 

In short, the latest version of the [Business Input plugin](/plugins/volkovlabs-static-datasource/) allows you to:
...
The [Business Variable](/plugins/volkovlabs-variable-panel/) panel builds on top of the native dashboard variables. It allows you to have dashboard filters in a separate panel, which you can place anywhere on the dashboard.
...
Among many beloved cases that this panel solves with ease is [dashboard redirect](https://volkovlabs.io/plugins/volkovlabs-variable-panel/redirect/). 

This feature allows the corresponding dashboard to be opened to a variable value. It is useful, for instance, if a variable panel displays a list of some devices where every device requires a specific dashboard.
...
This plugin is not yet available for the public, but it should be before the end of August. The main features will include:
 ...
 - ThreeView with expandable and collapsible elements,
 - Supporting tabs for multiple data frame views. 

## Mix and Match the Business Suite plugins

After three years of improvements and ensuring test coverage more than 95%, all our open-source plugins beautifully came together and formed the open-source Business Suite. The Business Suite is a collection of uniquely positioned plugins, each providing flexible solutions for a wide range of business tasks. 

Furthermore, in addition to using our plugins individually, you can combine them to achieve astonishing results. 

A great example of plugin combination is the file upload example. It is described in detail in one of our blog posts [here](https://volkovlabs.io/blog/form-panel-file-upload-20240310/).

<Youtube id="V4Sza0uDQNs" title="File Upload using Grafana dashboard | Data Manipulation | Base64 | Variable panel | Tutorial" />

There, three plugins are combined:
- Business Forms, to allow user choosing a file and then sending it to the data source,
- Business Variable, to list the uploaded files,
...
We carefully study all the questions we receive on our YouTube channel and GitHub repository and use many as inspiration.

Based on our three years of active interaction with the community, we can attest that those ideas will never cease to pour in.
