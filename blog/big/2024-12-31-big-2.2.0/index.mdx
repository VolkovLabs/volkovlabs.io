...
The Business Studio is an application developed by Volkov Labs. It plays the role of the control panel for the whole observability system, providing a user-friendly interface and keeping all necessary configurations in one place. 

You start by connecting to as many Business Engines as needed, where each one is connected to a Grafana instance. 

From there, an authorized user can:
 - create and manage alert rules and alert actions,
 - conduct Grafana version management,
 - propagate new dashboard versions,
...
You can download the Business Studio following the links provided at the [end](#business-studio-downloads) of this article. 
:::
...
If you need a token for more than 10 Business Engines, use the **Support** menu at the bottom left to request it. 
...
:::info limit 10 Business Engines
Please note that the Community version of the Business Studio supports no more than 10 connected Business Engines. 
:::

Once the Business Engine is added, you can review the available engine configuration and actions by clicking on its name in the list.

<Image
  title="Business Engine configuration."
...
2. Review the alert rule states for each existing alert rule. There are two possible states:
  - **Active**,
  - **Paused**. 
3. Review the alert rule status for each existing alert rule. There are five possible statuses:
  - **Scheduled**,
  - **OK**,
  - **Alerting**,
  - **Error**.
...
The new alert action/edit mode has the following configuration elements to specify:
 ...
 - **Title**, 
 - **Type** (HTTP or Logs),
 - **HTTP Request URL** and request **method**. You can add **header** (additional information) to a request if needed,
 - **Message**. It is customizable with Handlebars.

You can use the **Test Connectivity** button to check REST API endpoint availability.

### Supports for Light and Dark themes

The Business Studio follows your visual appearance preferences by supporting Light and Dark color themes.
...
We ensured the installation could be done in one click for MacOS, Linux, and Windows. 
For MacOS and Windows, you can configure the auto-update functionality to stay up-to-date with the latest release features

## Support of major Grafana 11 HTTP API

The Business Studio works with Grafana starting from version 11. It might work with Grafan 10 and prior, but we did not extensively test that compatibility. Use at your own risk.

The Business Studio will always be compatible with the most current Grafana version. 

## Default values for environment variables

To simplify the initial Business studio setup, we configured many of the environment variables by giving them default values. 

There are only 7 environment variables that must be setup after the installation.
...
The environment info is taken by using the [following operations](https://volkovlabs.io/big/api/#tag/Environment/operation/EnvironmentController_get).

All environment variables are displayed on the **Environment** tab. In the future, we plan to allow them to be modified. 

<Image
  title="Environment variables for the Charlies Engine 1."
  src="/img/blog/2024-12-31-big-2.2.0/env.png"
/>

## Added Engine API to get timeline of alert statuses

We added Engine API to [get timeline of alert statuses](https://volkovlabs.io/big/api/#tag/Alert-Rules/operation/AlertsController_getAllHistory). 

The timeline of alert statuses is a subset of all alert rule status records. It includes only instances where status changes.

For instance, below is the complete list of alert rule statuses. 
```text
date                      Previous            New
...
```
And here is the timeline. It consists of only the records where the status changes.
