...
The Business Studio is an application developed by Volkov Labs. It plays the role of the control center for the whole observability system, providing a user-friendly interface and keeping all necessary configurations in one place.
...
The environment info is taken by using the [following API endpoint](https://volkovlabs.io/big/api/#tag/Environment/operation/EnvironmentController_get).
...
We added Engine API endpoint to [get timeline of alert statuses](https://volkovlabs.io/big/api/#tag/Alert-Rules/operation/AlertsController_getHistoryTimeline). The timeline of alert statuses is a subset of all alert rule status records. It includes only instances where status changes.
