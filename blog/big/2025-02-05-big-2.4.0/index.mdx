...
Data routes updated from `/grafana` to `/grafana-data`.  
:::

### Business Studio 2.4.0

- Added Grafana connection management in Engine Environment.
- Enhanced alert event details with conditions and code view.
- Included backend query verification for panels.
- Added HA state to Engine Overview.
- Simplified alert scheduling with templates.
- Enabled data frame previews for alerts.
- Improved UI prompts for clearer configuration.

### Business Engine 2.4.0

- Added API for real-time Grafana config updates.
- Added API for alert event details and HA state.
