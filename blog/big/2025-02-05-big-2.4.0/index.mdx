...
:::info Multiple engine update

If multiple Business Engines are connected to the same Grafana instance (HA scenario), then updating ONLY one Business Engine is required. The rest of the connected business engine will be updated automatically.
...
:::info terminology
An alert event is an alternative term for an [alert record](/big/alerting/#alert-components). Technically, it is a record in a database about the alert breach event.
:::

In the Business Studio, you can review:

- all alert events in one listing. For that:
...
<Image
  title="Review all alert events in one listing."
  src="/img/blog/2025-02-05-big-2.4.0/all-alerts.png"
/>

- one alert event history. It is better for analyzing state changes for one alert rule.
...
<Image
  title="Review one alert event history."
  src="/img/blog/2025-02-05-big-2.4.0/one-alert.png"
/>

You can click on any alert record to review details.

If the latest alert state (**New state**) is **Alerting**, the **Alert evaluations** section shows the values that triggered the alert action. You always can review the payload by clicking on the **View as code** button.
...
- **Disabled**. If a Business Engine is the only one in the cluster (only one Business Engine is connected to a Grafana instance).
- **Enabled**. If there are multiple (more than one) Business Engines in the cluster (more than one Business Engine are connected to a Grafana instance).

<Image
  title="High availability characteristic for a Business Engine."
...
We maintain the ease of the 3rd party Business Engine integration by timely updating the [API documentation](/big/api/).
