...
tags: [Data Manipulation, Dynamic Form Elements]
...
# How to build Form Elements dynamically
...
The Form Elements are the interface input/output built-in types that you can use to create your unique Data Manipulation form. As of today, you have 16 types at your disposal.

<Image
  title="Some examples of the Form Element types on UI."
...
- **Static**. The Data Manipulation form allows you to set the Form elements manually using the plugin options. Below is an example of what parameters you can control manually for the **Select with custom options** type.
...
- **Dynamic**. The Data Manipulation form supports a Form Element placement via code or, in other words, programmatically. This approach is for more advanced users and can be used in the following cases:
 
   - The Data Manipulation form needs to contain a large number of Form Elements, making manual configuration too tedious. 
...
<Image
  title="Configured Device dashboard variable."
  src="/img/blog/2024-04-28-form-panel-server-based-elements/device-variable.png"
/>

### Data Manipulation

The configuration of the Data Manipulation consists of two logical parts:
...
With that, the Custom code could be as follows. Here, we parse the JSON description and convert it into the commands to create actual Form Elements. 
Note, that we adjusted helpers to ensure the code works correctly.

<Image
  title="Initial request configuration."
...
#### BONUS 1 Update the dashboard variable following the form element changes
...
#### BONUS 2 Keeping comment value on the device change
