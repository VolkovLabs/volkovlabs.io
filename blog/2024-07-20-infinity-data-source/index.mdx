...
This article provides an example of how the [Infinity](https://grafana.com/grafana/plugins/yesoreyeram-infinity-datasource/) data source can be integrated with the [Business Forms](/plugins/volkovlabs-form-panel/) plugin on the Grafana dashboard.
...
Below is the illustration of the **Initial Request Payload** configuration. I keep the most parameters by default. The **URL** is any WHAT????
...
Using `console.log('context:', context)` you can find more available options and methods.
...
My payload uses panel element ID to find the corresponding values and send them into the payload.
...
- Set **URL** set to any URL that WHAT??????
