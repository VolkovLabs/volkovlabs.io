---
authors: [daria]
slug: big-alerting-20231113/
tags: [Business Intelligence]
image: /img/blog/2023-11-13-big-alerting/banner.png
...
Most asks were related to simplifying user interaction by possibly having all controls in one UI place. Many users were eager to incorporate forecasting paired with AI.

This is how the idea of the Big Alerting was born. With the general thought of making alerting accessible for business users.

In the video below, I go over the alerting basics, existing alerting system components and our vision of BIG Alerting. 
...
<Video
  ...
  title="BIG alerting announcement"
/>
...
<Image
  ...
  src="/img/blog/2023-11-13-big-alerting/basics.png"
  width="50%"
/>
...
## Alerting versus BIG alerting

The schema below depicts the Alerting as it is side-by-side with the Big Alerting, so you can see the similarities and differences. Each of the main alerting components (rule, record, and action) has a corresponding software module.

Users create alert rules using the Alerting UI.

<Image
  title="Alerting versus BIG alerting."
  src="/img/blog/2023-11-13-big-alerting/alerting-vs-big.png"
/>
...
For the alert actions, Grafana has an extensive notification alerting channel system. Based on the amount of questions we received and came across, it has a steep learning curve. It allows you to set the channels to configure sending text, slack messages and emails. 
...
In the Big Alerting, we reimaged all three modules. 
...
<Image
  ...
  src="/img/blog/2023-11-13-big-alerting/alert-management-ui.png"
/>

### Alerting BI Engine

We reimagined the alert manager and came up with the BI Alert Engine:
 ...
 - It is installed in a separate container which makes the system architecture flexible and performance improvement easier. 
...
<Image
  ...
  src="/img/blog/2023-11-13-big-alerting/bi-ds.png"
/>
...
<Image
  ...
  src="/img/blog/2023-11-13-big-alerting/web-hooks-panel.png"
/>

## Product POC (proof of concept)

You can download the PRODUCT POC from our [GitHub repository](https://github.com/volkovlabs/volkovlabs-bi-toolkit). The docker-compose file describes the following containers:

- Grafana container. It includes the provisioned BI Engine data source, an Alerting panel, and an example dashboard.
- BI Engine container. It has a service account key to access Grafana HTTP APIs. It evaluates alert rules and calls webhooks when alert statuses change.
- JSON web-hook container. It is a web-hook example based on nodeJS, which accepts alert payload and saves it to the files for testing purposes.
- Node-RED container provides an HTTP POST endpoint and sends a Slack notification with alert details.

When you run a docker-compose file, it launches the Grafana and BI Engine containers. 

## Early adapters

Reach out immediately if you are interested in the product roadmap discussion. We are happy to gather a group of early adopters to swiftly push this product forward. 

The first official version is planned on January '24, and there is so much more to come afterwards. Together we can ensure the product includes the most desired features. 

## Questions?

If you have any questions about this article, please, do not hesitate to leave them in the comments section under the [YouTube video mentioned at the beginning](https://www.youtube.com/embed/vky-7-DfvXE).
