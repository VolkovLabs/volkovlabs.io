...
- **Markdown & Handlebars**: Craft rich layouts with Markdown and inject live data using Handlebars templating—ideal for reports or custom displays.
- **JavaScript**: Add custom JavaScript (pre- or post-render) to manipulate data, query APIs, or integrate libraries, unlocking endless possibilities.
- **CSS**: Style your text with custom CSS and Grafana variables for a branded, professional look.
- **Global Helpers**: Format time ranges like `__from` and `__to`, or use built-in helpers for nested objects and localization.
