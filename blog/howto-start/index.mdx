...
Over time, the [Business Suite for Grafana](/plugins) might seem like it became a massive, multi-faceted entity with diverse functionality and plenty of nuances. 

This article is due to clarify the basics and give a quick cheat sheet with clear and easy steps on how to start. Hint: it is easier than you might think.

I will walk you through the following phases of working with the Business Suite collection: s
...
Let me start with the schema summarizing the installation scenarios. I categorized them for a Cloud account and OSS/Enterprise instances.
...
3. Optionally, enter the plugin name(or partial name) into the **Search** box. The word *Business* filters all ten plugins of the Business Suite plugin (as shown).
...
8. Use the action buttons to upgrade/degrade/delete the selected plugin.
...
4. Optionally, enter the plugin name(or partial name) into the **Search** box. The word *Business* filters all ten plugins of the Business Suite plugin (as shown).
...
This approach is irreplaceable for air-gapped environments (networks not connected to the www due to various security rules).
...
2. Go into the **Products** menu at the top of the screen and select the **Plugins** category.
3. Optionally, enter the plugin name (or partial name) into the **Search** box. The word *Business* filters all 10 plugins of the Business Suite plugin (as shown).
4. Click on the plugin card of your choice.
...
Usually, you use the [CLI](https://grafana.com/docs/grafana/latest/cli/) (command line Grafana tool) to install any open-source plugins available in the Grafana catalog. 

In addition, with CLI, you can install Grafana plugins that are NOT in the Grafana catalog. 

For instance, our [Environment Data source](/grafana/environment/) can not be accepted into the catalog due to its functional ability to expose specific system settings that should be hidden.

However, thousands of users use this plugin precisely for that reason, namely, to display system settings on Grafana dashboard for administrative purposes.
...
After a data source plugin is installed, you need to add it as a data source to your Grafana. 
...
To go deeper into the Grafana transformation subject, watch the following YouTube video and read the post I published on our blog.
...
The [Business News](https://volkovlabs.io/plugins/business-news/) data source converts the RSS/Atom news feeds into the Grafana data frames. Combining it with the [Business Text panel](https://volkovlabs.io/plugins/business-text/) lets you visualize web resource updates on your Grafana dashboard in any CSS/HTML style.
...
Even though both plugins (the [Business Variable](https://volkovlabs.io/plugins/business-variable/) and [Business Table](https://volkovlabs.io/plugins/business-table/)) are visualization panels, they can work together in creating beautiful workflow for your users. 
...
### Business Media plus Business Forms (Upload a file)

Uploading a file from a computer via the Grafana dashboard into the connected data source is one of the most common tasks. Using the [Business Media](https://volkovlabs.io/plugins/business-media/) panel along with the [Business Forms](https://volkovlabs.io/plugins/business-forms/) makes it possible in minutes. To simplify this method and help its adoption, we posted an article on our blog and produced a YouTube video walking through all the necessary steps. 
...
### Infinity data source plus Business Charts

An example of the [Infinity data source](https://grafana.com/grafana/plugins/yesoreyeram-infinity-datasource/) combination with the [Business Charts](https://volkovlabs.io/plugins/business-charts/) canbe found in the YouTube tutorial below.

<Youtube
  id="B4Uj1n4Cr88"
  title="JSON API and Infinity Data Source tutorial for Grafana | How to display the unemployment rate."
/>
...
There are two main channels for our open-source plugins to receive a change request or have your questions answered.

### YouTube comments

Leave a comment with your question under any of our YouTube videos.

We prioritize topics that could potentially benefit a broad audience. If you have anything specific or need an answer quickly, feel free to use our [GitHub sponsor](https://github.com/sponsors/VolkovLabs) option.

### GitHub requests

If you notice something that can be improved in our plugins or believe you need a particular change, you can request it on our GitHub repository. 
