...
Out of the box, Grafana allows adding native annotations, alert rules for your time series, and visualize annotation queries.
...
<Image
  title="Alerting rules are periodically evaluated and fire if the threshold unmet."
...
Alternatively, you can go with a configuration database to capture your annotations. In that event, annotations queries could be divided into two types, built-in and user-defined.
...
The built-in Annotation Query does not provide features like filters and support for dashboard variables.
...
However, when you display data points from multiple devices with user-defined annotations queries, the broken heart emoji might be a misleading overkill. The broken heart emoji will not be displayed if you use Grafana HTTP API Data Source and configure dashboard filters.
