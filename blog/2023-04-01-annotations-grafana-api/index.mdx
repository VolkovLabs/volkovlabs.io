...
Annotation queries are an advanced way to work with annotation records. You can store annotation records in a database like PostgreSQL or any other storage of your choice and access the using corresponding data sources.
...
However, when you display data points coming from multiple devices with user-defined annotations queries, the broken heart emoji might be a misleading overkill. The broken heart emoji will not be displayed if you use Grafana HTTP API Data Source and configure dashboard filters.
